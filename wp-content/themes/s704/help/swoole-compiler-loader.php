...
if ('web' == $env['php']['run_mode']) {
    ...
    if (empty($html_error)) {
        ...
        $html_body_loader .= '<p><span class="bold_text">1 - 下载Swoole Loader</span></p><p>请下载找到兼容PHP' . $env['php']['version'] . '和' . $env['php']['thread_safety'] . '的Swoole Loader扩展，<a target="_blank" href="https://www.lanzous.com/i7w9led">点击直达下载页面</a></p>';
        $html_body_loader .= '<p><span class="bold_text">2 - 安装Swoole Loader</span></p><p>将刚才下载的Swoole Loader扩展文件（swoole_loader版本号.dll或swoole_loader版本号.so）上传到当前PHP的扩展安装目录中：<br/><pre class="code">' . $env['php']['extension_dir'] . '</pre></p>';
